...
data class TempUiState(
    ...
    val roomName: String = "Đang tải...", // [Thêm mới] Tên phòng
    val sensors: List<TempSensor> = emptyList(), // [Cập nhật] Model thật
...
class TemperatureViewModel(
    ...
    private fun loadInitialData() {
        viewModelScope.launch {
            ...
            // 2. Lấy danh sách cảm biến
            repository.getTempSensors(roomId).collect { result ->
                when (result) {
                    is NetworkResult.Success -> {
                        ...
                        _uiState.update {
                            it.copy(
                                isLoading = false,
                                roomName = name, // Cập nhật tên phòng
                                sensors = sensors
                            )
                        }

                        // Mặc định chọn tất cả để vẽ biểu đồ
    ...
    // ... (Các hàm loadChartData, onSensorSelected, DateChange giữ nguyên logic cũ) ...
    // Chỉ cần lưu ý format ngày khi gọi API getTempHistory
            ...
            repository.getTempHistory(roomId, startStr, endStr).collect { res ->
                when (res) {
                    is NetworkResult.Success -> {
                        ...
                        // [LOGIC MỚI] Tạo Formatter ngày giờ
                        // Pattern: ngày/tháng giờ:phút (VD: 20/11 14:00)
                        val formatter = DateTimeFormatter.ofPattern("dd/MM HH:mm")
                        val zoneId = ZoneId.systemDefault() // Múi giờ người dùng

                        // Map dữ liệu
                        val entries = historyList.mapIndexed { index, item ->
                            FloatEntry(x = index.toFloat(), y = item.avgTempC.toFloat())
                        }
